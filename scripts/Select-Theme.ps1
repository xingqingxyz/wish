...
param (
  [ArgumentCompleter({
      ...
      ('alacritty', 'bat', 'delta', 'windows_terminal').Where{ $_ -like "$WordToComplete*" }
    })]
...
$themes = switch ($AppName) {
  alacritty {
    Split-Path -Resolve -LeafBase $env:WISH_ROOT/alacritty-theme/themes/*; break
  }
  bat {
    bat --list-themes; break
  }
  delta {
    @(delta --list-syntax-themes).ForEach{ $_.Split(' ')[-1] }; break
  }
  windows_terminal {
    'CGA', 'Campbell', 'Campbell Powershell', 'Dark+', 'Dimidium', 'IBM 5153', 'One Half Dark', 'One Half Light', 'Ottosson', 'Solarized Dark', 'Solarized Light', 'Tango Dark', 'Tango Light', 'VSCode Dark Modern', 'VSCode Light Modern', 'Vintage'; break
  }
  # no default
}
try {
